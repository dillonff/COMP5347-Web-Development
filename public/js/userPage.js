...
function fillPhoneReviews() {
  ...
  tbody_tag.innerHTML = "";
  for (let i = 0; i < relatedPhongListings.length; i++) {
    let create_phonereview_row = document.createElement("tr");
    create_phonereview_row.setAttribute("id", "phone" + i);
    create_phonereview_row.innerHTML =
      '<td class="phone-title">' +
      relatedPhongListings[i].title +
      "</td>" +
      "<td>" +
      ...
      '"/></td>' +
      "<td>" +
      relatedPhongListings[i].reviews +
      "</td>";
    tbody_tag.appendChild(create_phonereview_row);
  }
}
